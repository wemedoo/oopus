...
namespace sReportsV2.DTOs.ThesaurusEntry.DataOut
{
    public class ThesaurusEntryDataOut
    {
        ...
        {
            return GetTranslationByLanguage(language) ?? GetTranslationByLanguage(LanguageConstants.EN) ?? Translations.FirstOrDefault(x => x.Language != null) ?? new ThesaurusEntryTranslationDTO() { Abbreviations = new List<string>(), Synonyms = new List<string>()};
        }
