    ...
    public class FormDAL : IFormDAL
    {
        ...
        private IQueryable<Form> SortByField(IQueryable<Form> result, FormFilterData filterData)
        {
            switch (filterData.ColumnName)
            {
                ...
                case AttributeNames.Class:
                    if (filterData.IsAscending)
                        return result.ToList().OrderBy(x => x.DocumentProperties.Class?.Class?.ToString())
                                .Skip((filterData.Page - 1) * filterData.PageSize)
                                .Take(filterData.PageSize).AsQueryable();
                    else
                        return result.ToList().OrderByDescending(x => x.DocumentProperties.Class?.Class?.ToString())
                                .Skip((filterData.Page - 1) * filterData.PageSize)
                                .Take(filterData.PageSize).AsQueryable();
                case AttributeNames.ExplicitPurpose:
                    if (filterData.IsAscending)
                        return result.ToList().OrderBy(x => x.DocumentProperties.Purpose?.ExplicitPurpose.ToString())
                                .Skip((filterData.Page - 1) * filterData.PageSize)
                                .Take(filterData.PageSize).AsQueryable();
                    else
                        return result.ToList().OrderByDescending(x => x.DocumentProperties.Purpose?.ExplicitPurpose.ToString())
                                .Skip((filterData.Page - 1) * filterData.PageSize)
                                .Take(filterData.PageSize).AsQueryable();
                case AttributeNames.ClinicalContext:
                    if (filterData.IsAscending)
                        return result.ToList().OrderBy(x => x.DocumentProperties.ClinicalContext?.ClinicalContext.ToString())
                                .Skip((filterData.Page - 1) * filterData.PageSize)
                                .Take(filterData.PageSize).AsQueryable();
                    else
                        return result.ToList().OrderByDescending(x => x.DocumentProperties.ClinicalContext?.ClinicalContext.ToString())
                                .Skip((filterData.Page - 1) * filterData.PageSize)
                                .Take(filterData.PageSize).AsQueryable();
                default:
        ...
        public bool IsNullFlavorUsedInAnyField(string formId, int nullFlavorId)
        {
            return Collection.AsQueryable().FirstOrDefault(x => x.Id.Equals(formId) && !x.IsDeleted)?
                .GetAllFields()?.Any(field => field.NullFlavors.Contains(nullFlavorId)) ?? false;
        }
