...
//            form.Language = LanguageConstants.EN;
...
//            if (!enumService.GetAll().Where(x=>x.Type==IdentifierKind.PatientIdentifierType.ToString()).Any(x => x.EntityStateCD != (int)EntityStateCode.Deleted && x.ThesaurusId == thesaurusId))
