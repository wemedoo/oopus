    ...
    $.fn.imageMaps = function(options) {
        ...
        this.each(function (i, item) {
            ...
            $(options.addBtn+':eq('+ i +')').unbind('click').on('click', function(){
                ...
                //let oDiv = $(`<div class="image-map-rect" id="imagemaps-rect-${i}-${index}" style="width:${(coords[2] - coords[0]) || options.rectWidth}px;height:${(coords[3] - coords[1]) || options.rectHeight}px;position:absolute;left:${coords[0] || 0}px;top:${coords[1] || 0}px;" data-index="${index}"><div class="rect-elements"><div class="count">${count}</div></div><div class="rotate-left-button"><img src="/css/img/icons/rotate_left.svg"/></div></div>`);
